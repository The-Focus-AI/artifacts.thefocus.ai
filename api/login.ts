...
export async function handleLoginRequest(
  ...
  // Redirect to Clerk sign-in if no session token is present
  if (!url.searchParams.has("__clerk_db_jwt")) {
    const signInUrl = deps.clerk.buildSignInUrl(url.toString());
    redirect(nodeResponse, signInUrl);
    return;
  }

  // Clerk callback: verify the session and check email domain
  ...
  // Fallback: display token on page if no localhost callback port
